...
int main(){
    ...
    printf("Enter the at and corresponding bt of each process : \n");
    for(int i = 0 ; i < n ; i++){
        a[i].pid = i+1;
    ...
    while(completed != n){
        int current_process_index = -1, min_bt = INT_MAX;
        for(int i = 0 ; i < n ; i++){   //finding process with minimum bt
        ...
        }//Now current_process_index contains index of process with min rem bt
